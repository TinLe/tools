...
def dump_instance_data(instance, region='us-east-2'):
...
@click.option('--region', '-r',
        default='us-east-2',
        help='AWS Region to read data from (default to us-east-2).')
def main(pattern, debug, verbose, hosts, platform, subnet, region):
  ...
  NOTE: requires python3+ and boto3 (for AWS supports)
    ...
    try:
        ...
        if debug:
            # response_info = json.load(response)['Reservations']
            # pp.pprint("ec2.describe_instances: {0}".format(response_info))
            pp.pprint("ec2.describe_instances: {0}".format(response))
        if subnet:
            print("SubnetId,VpcId,NI-Id,SourceDestCheck,PublicDns,PublicIp,Primary,PrivateIp,PrivateDns")
        if hosts:
